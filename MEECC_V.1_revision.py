...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 06:20:52 2026
...
ui_to_data_map = {
    "Per Capita GDP": "PCGDP_2019_$/perons -2017PPP",
    "Per Capita Primary Energy Consumption": "PPE_2019_GJ/p",
    "Access to Electricity": "ELEC_2019_%",
    "Access to Clean Cooking Fuels": "CCF_2019_%",
    "Life expectancy at birth": "LEX_2019_years",
    "Infant mortality rate": "IMR_2019_Deaths per 100 live births",
    "Maternal Mortality Ratio": "MMR_2017_Women dealths per 100,000 live births",
    "Share of deaths from Air Pollution": "DAP_2019_%",
    "Daily Calorific Intake": "DAC_2018_kcal/p/day",
    "Mean years of schooling": "MYS_2019_years",
    "Access to Internet": "IAC_2019_%",
    "Deaths due to unsafe sanitation": "DUS_2019_deaths/100,000 people",
    "Share of the urban population living in slums": "PLS_2018_% of urban population",
    "Death rate from unsafe water sources": "DUW_2019_Deaths per 100,000 people",
    "Per capita CO2-FFI": "PCO2-FFI_2019_tCO2/person",
    "Distance from fair share of cumulative CO2-FFI":"CCO2-FF1_DFS_2019_GtCO2"
}
# Build dataset using ONLY mapped variables
all_cols = [ui_to_data_map[v] for v in ui_to_data_map.keys()]

data_eq_weight = data[all_cols]

# Normalize once
data_eq_weight_norm = (data_eq_weight - data_eq_weight.min()) / (
    data_eq_weight.max() - data_eq_weight.min()
)

# Default equal weighting (so app loads before user inputs)
data["All_indicators"] = data_eq_weight_norm.mean(axis=1)

# Initial clustering (required for app to work)
...
pcgdp_ui = "Per Capita GDP"
pcgdp_col = ui_to_data_map[pcgdp_ui]
...
app.layout = html.Div([
    ...
    html.Div([
            ...
            dcc.Tabs(id="main-tabs", value="clustering_method", children=[
                ...
                dcc.Tab(label="Clustering Method", value="clustering_method", children=[
                    html.Div([
                        ...
                        html.Div(id="all-dev-weight-container"),
                        html.Button(
                            "Apply Weights",
                ...
                dcc.Tab(label="Developmental Scenarios", value="developmental_scenarios", children=[
                    html.Div([
                        ...
                        html.Label("Peak Year Mode:"),
                        
                        dcc.Dropdown(
                            id="peak-year-mode",
                            options=[
                                {"label": "Historical (Model-based)", "value": "historical"},
                        ...
                        html.Div(
                            id="user-peak-year-container",
                            children=[
                                html.Label("User Defined Peak Years (by Group):"),
                                ...
                                html.Div([
                                    html.Div([
                                        ...
                                        dcc.Dropdown(
                                            id="peak-year-g1",
                                            options=[{"label": str(y), "value": y} for y in range(2020, 2051)],
                                            value=2020,
                                            clearable=False
                                        )
                                    ]),
                                ...
                                style={
                                    "display": "grid",
                                    "gridTemplateColumns": "repeat(5, 1fr)",  # 3 per row
                                    "gap": "10px",
                                    "marginTop": "10px"
                                })
                            ]
                        ...
                        html.Label("Minimum Gap (Peak → Net Zero):"),
                        
                        dcc.Dropdown(
                            id="peak-nzy-gap",
                            options=[{"label": str(x), "value": x} for x in [5,10,15,20,25,30]],
                            value=5
                        ),
        ...
        html.Div([
            ...
            html.Div(
                id="selected-country-group-display",
                style={
                    "marginTop": "10px",
                    "fontWeight": "bold",
                    "color": "#333"
...
@app.callback(
    ...
    Input("clustering-method-output", "children")
)
def update_country_options(selected_method,_):
...
@app.callback(
    Output("all-dev-weight-container", "children"),
    Input("clustering-method-radio", "value")
)

def show_weight_inputs(method):

    if method != "equal_weighting":
        return None

    non_pcgdp_vars = [v for v in ui_to_data_map.keys() if v != pcgdp_ui]

    return html.Div([
        html.H4("Assign Weights (Total ≤ 1)"),

        html.Div([
            html.Div([
                html.Label(var),
                dcc.Input(
                    id={"type": "weight-input", "index": var},
                    type="number",
                    min=0,
                    max=1,
                    step=0.01,
        ...
        html.Label("Per Capita GDP Weight (auto)"),
        dcc.Input(
            id="pcgdp-weight",
            type="number",
            readOnly=True,
        ...
        html.Div(id="weight-warning", style={"color": "red"})
    ])

@app.callback(
    Output("pcgdp-weight", "value"),
    Output("weight-warning", "children"),
    Input({"type": "weight-input", "index": ALL}, "value")
)
def update_pcgdp_weight(weights):

    weights = [w if w else 0 for w in weights]
    total = sum(weights)

    remaining = 1 - total

    if remaining < 0:
        return 0, "⚠ Total weight exceeds 1"
...
@app.callback(
    Output("clustering-method-output", "children"),

    # 🔥 ONLY trigger
    Input("apply-weights-btn", "n_clicks"),

    # 👇 read values as STATE
    State({"type": "weight-input", "index": ALL}, "value"),
    State("pcgdp-weight", "value"),
    State("clustering-method-radio", "value")
)
def update_weights(n_clicks, weights, pcgdp_weight, method):

    print("🔥 update_weights triggered")

    if method != "equal_weighting":
        return ""

    if not weights:
        return "⚠ No weights detected"

    weights = [w if w else 0 for w in weights]

    non_pcgdp_ui = [v for v in ui_to_data_map.keys() if v != pcgdp_ui]
    non_pcgdp_cols = [ui_to_data_map[v] for v in non_pcgdp_ui]

    data_normed = data_eq_weight_norm.copy()

    weight_dict = dict(zip(non_pcgdp_cols, weights))

    weighted_sum = sum(
        weight_dict[col] * data_normed[col]
        for col in non_pcgdp_cols
    )

    data["All_indicators"] = weighted_sum + pcgdp_weight * data_normed[pcgdp_col]

    old_clusters = data["cluster_Eq_weight"].copy()
    data["cluster_Eq_weight"] = assign_cluster(data["All_indicators"])
    
    changed = data[old_clusters != data["cluster_Eq_weight"]]

    if not changed.empty:
        print("\n🔁 Cluster changes (Country: Old → New):")
        for idx in changed.index:
            country = data.loc[idx, "Countries"]
            old = old_clusters.loc[idx]
            new = data.loc[idx, "cluster_Eq_weight"]
            print(f"{country}: G{old} → G{new}")
    else:
        print("\nNo cluster changes")

    return f"✔ Updated | PCGDP weight = {round(pcgdp_weight,4)}"



@app.callback(
    Output("user-peak-year-container", "style"),
    Input("peak-year-mode", "value")
)
def show_hide_user_peak(mode):
...
@app.callback(
    [Output('result-graph', 'figure'), Output("data-table", "data"), 
     ...
     Output("country-limit-warning", "children"), Output("selected-country-group-display", "children")],
    [Input('country-checkbox', 'value'),
     ...
     Input("clustering-method-output", "children"),
     Input("peak-year-mode", "value"),
     Input("peak-year-g1", "value"),
     Input("peak-year-g2", "value"),
     Input("peak-year-g3", "value"),
     Input("peak-year-g4", "value"),
     Input("peak-year-g5", "value"),
     Input("peak-nzy-gap", "value")
     ]
)
def update_graph(selected_countries, selected_gdpgrowth, selected_energythreshold,
                 ...
                 selected_temp_target, selected_figure, selected_variable, selected_method, clustering_trigger, peak_year_mode, peak_g1, peak_g2, peak_g3, peak_g4, peak_g5, peak_nzy_gap):
...
# Step 2: Process each selected country separately
    for country, groups in all_selected_countries.items():
        ...
        if peak_year_mode == "user_defined":
            peak_year = group_peak_year.get(cluster)
        
            if peak_year is None:
                peak_year = 2030
        
        elif peak_year_mode == "historical":
        ...
        print(f"{country} | Peak Year: {peak_year}")
        # Now recalculate NZY
        peak_year_df=energy_project_df.copy()
        peak_row = peak_year_df.loc[peak_year_df["Years"] == peak_year]

        if peak_row.empty:
            peak_year = 2030  # fallback
            peak_row = peak_year_df.loc[peak_year_df["Years"] == peak_year]
        
        Peakyear_emissions = peak_row["Net_CO2_Baseline"].values[0]
        ...
        gap = peak_nzy_gap if peak_nzy_gap is not None else 5
        if NZY is None or (NZY - peak_year) <= gap:
            Annual_reduction_rate = None
            NZY=None
...
# =============================================================================
    ...
    # Build display text: Country → Group
    display_list = []
    
    for country in all_selected_countries.keys():
        country_data = data[data["Countries"] == country].iloc[0]
        ...
        display_list.append(f"{country} → G{cluster}")
    
    # Final string
    group_display_text = " | ".join(display_list)
    
    return fig, table_data,table_columns, variable_bar_figure, warning_message, group_display_text
